...
class EncodingJStringWriter(ctx: JEncodingContext) extends JStringWriter {
  ...
  override def write(c: Int): Unit = {
    guard()
    EncodingJStringWriter.encodeFragment(c.toChar, ctx.writer, ctx.options)
  }

  override def write(chars: Array[Char], offset: Int, length: Int): Unit = {
    guard()
    EncodingJStringWriter.encodeFragment(chars, offset, length, ctx.writer, ctx.options)
  }
